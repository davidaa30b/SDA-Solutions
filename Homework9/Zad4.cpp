...
unordered_map<char, unsigned long long>decodingTable;
...
struct RollingHash {
    ...
    RollingHash(const string& s) : prefixes1(s.size() + 1), prefixes2(s.size() + 1) {
        ...
        for (int i = 0; i < n; ++i) {
            prefixes1[i + 1] = (prefixes1[i] + 1LL * decodingTable[s[i]] * powers1[i]) % mod;
            prefixes2[i + 1] = prefixes2[i] + decodingTable[s[i]] * powers2[i];
        }
    }
